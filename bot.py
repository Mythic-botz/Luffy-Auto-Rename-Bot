...
class UserDownloadManager:
    def __init__(self):
        self.user_queues = {}  # Dictionary to store queues per user
        self.user_semaphores = {}  # Dictionary to store semaphores per user
        self.max_concurrent = 5  # Maximum concurrent tasks per user
    ...
    async def process_queue(self, user_id):
        ...
        while not queue.empty():
            async with semaphore:  # Limit to max_concurrent tasks
...
async def process_file(client, message):
    ...
    # Use a simpler check to avoid duplicate processing
